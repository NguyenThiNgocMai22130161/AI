...
public class Puzzle {
	...
	public int computeH2(Node currentState) {
		...
		for (int tile = 1; tile < MAX_ROW * MAX_COL; tile++) {
				int[] CountPos = currentState.getLocation(tile);
				int[] GoalPos = goalState.getLocation(tile);
		        result += PuzzleUtils.manhattanDistance(CountPos, GoalPos);
		}
		        return result;
	}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			if (row >= 0) {// Tile stands inside the map
				...
				result.setG(currentState.getG()+1);
				return result;
			}
		}

		else if (operator == 'd') {// Case-2: Move tile DOWN //xuống ô dưới
			int row = whiteTile[0] + 1;
			int col = whiteTile[1];
			if (col < MAX_ROW) { //ô trống nằm trong bản đồ
		...
		else if (operator == 'l') {// Case-3: Move tile LEFT //qua trái
			int row = whiteTile[0];
			int col = whiteTile[1] - 1;
			if (col >= 0) { //ô trống nằm trong bản đồ
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, col, 0);
				result.updateTile(whiteTile[0], whiteTile[1], tmp);
				result.setH(computeH2(result));
		...
		else if (operator == 'r') {// Case-4: Move tile RIGHT //qua phải
			int row = whiteTile[0];
			int col = whiteTile[1] + 1;
			
            if (col < MAX_COL) { //ô trống nằm trong bản đồ
            	int tmp = currentState.getTile(row, col);
            	result.updateTile(row, col, 0);
            	result.updateTile(whiteTile[0], whiteTile[1], tmp);
            	result.setH(computeH2(result));
